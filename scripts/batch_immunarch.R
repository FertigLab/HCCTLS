# Plot distribution of CDR3 lenths for all samples, including by patient
exploratory <- function(x) {
  ...
  my_xaxis = list(
    ...
    scale_x_discrete(
      labels= function(y) x$meta$patient_tls_pub_id[match(y, x$meta$Sample)],
  ...
  plot2<-lapply(1:length(meta.subset), function (i) vis(p1, .by = meta.subset[i], .meta = x$meta, .test=T)+theme_prism())
  
  exp_len <- repExplore(x$data, .method = "len", .col = "aa")
  plot3<-vis(exp_len)
  plot4<-lapply(1:length(meta.subset), function (i) vis(exp_len, .by = meta.subset[i], .meta = x$meta)+theme_prism())
  
  exp_cnt <- repExplore(x$data, .method = "count")
  plot5<-vis(exp_cnt)
  plot6<-lapply(1:length(meta.subset), function (i) vis(exp_cnt, .by=meta.subset[i], .meta = x$meta, .test=T)+theme_prism())
  
  exp_vol <- repExplore(x$data, .method = "volume")
  plot7<-vis(exp_vol)+my_xaxis+  theme(legend.position="NULL")
  plot8<-lapply(1:length(meta.subset), function (i) vis(exp_vol, .by = meta.subset[i], .meta = x$meta, .test=T)+theme_prism())

  pdf(paste0(output.path,"exploratory.pdf"), width=9, height=7)
  print(plot1)
  print(plot2)
  print(plot3)
  print(plot4)
...
clonality <- function(x) {
  ...
  plot1 <-vis(imm_pr)+my_xaxis+  theme(legend.position="NULL")
  plot2 <- lapply(1:length(meta.subset), function (i) 
    vis(imm_pr, .by = meta.subset[i], .meta = x$meta)+theme_prism())
  ...
  plot3 <- vis(imm_top)+my_xaxis
  plot4 <- lapply(1:length(meta.subset), function (i) 
    vis(imm_top, .by = meta.subset[i], .meta = x$meta)+theme_prism())
  # While the rare method deals with the least prolific clonotypes:
  imm_rare <- repClonality(x$data, .method = "rare")
  plot5 <- vis(imm_rare)+my_xaxis+my_xaxis#+ggtitle("", subtitle="")
  plot6 <- lapply(1:length(meta.subset), function (i) 
    vis(imm_rare, .by = meta.subset[i], .meta = x$meta)+theme_prism())
  ...
  plot7 <- vis(imm_hom)+my_xaxis
  plot8 <- lapply(1:length(meta.subset), function (i) 
    vis(imm_hom, .by = meta.subset[i], .meta = x$meta)+theme_prism())

  pdf(paste0(output.path,"clonality.pdf"), width=9, height=7)
  print(plot1)
  print(plot2)
  print(plot3)
  print(plot4)
...
repOv <- function(x) {
  ...
  plot1 <- vis(imm_ov1, .text.size=2)+
    my_xaxis+
    scale_y_discrete(labels= function(y) x$meta$patient_tls_pub_id[match(y, x$meta$Sample)],
                     limits = rev(x$meta$Sample))

  #Public method repertoire overlap by patient
  imm_ov2 <- repOverlap(x$data, .method = "morisita", .col="aa")
  plot2 <- vis(imm_ov2, .text.size = 2)+
    my_xaxis+
    sc+
    scale_y_discrete(labels= function(y) x$meta$patient_tls_pub_id[match(y, x$meta$Sample)],
                     limits = rev(x$meta$Sample)) +ggtitle("Morisita overlap by CDR3aa")
  ...
  imm_ov2.ds <- repOverlap(x$data, .method = "morisita", .col="aa",.downsample=T)
  plot2.ds <- vis(imm_ov2.ds, .text.size = 2)+
      my_xaxis+
    sc+
      scale_y_discrete(labels= function(y) x$meta$patient_tls_pub_id[match(y, x$meta$Sample)],
                       limits = rev(x$meta$Sample)) +ggtitle("Downsampled Morisita overlap by CDR3aa")
    
  imm_ov2.vaa_ds <- repOverlap(x$data, .method = "morisita", .col="aa+v",.downsample=T)
  plot2.vaa_ds<- vis(imm_ov2.vaa_ds, .text.size = 2)+
      my_xaxis+
    sc+
      scale_y_discrete(labels= function(y) x$meta$patient_tls_pub_id[match(y, x$meta$Sample)],
                       limits = rev(x$meta$Sample)) +ggtitle("Downsampled Morisita overlap by v+aa")
  
  #export immune overlap object for subsequent plotting of overlap across patients
  if (tcrbcr == "TCRB") (
    if (str_detect(output.path, "downsampled")==T) {
      assign("imm_ov2_tcr_downsampled", imm_ov2, envir = .GlobalEnv)} 
    else {
      assign("imm_ov2_tcr", imm_ov2, envir = .GlobalEnv)
    }) 
  if (tcrbcr == "IGH") (
    if (str_detect(output.path, "downsampled")==F) {
      assign("imm_ov2_bcr_downsampled", imm_ov2 ,envir = .GlobalEnv)} 
    else {
      assign("imm_ov2_bcr", imm_ov2 ,envir = .GlobalEnv)
    })
  
  #morisita overlap for nt, v gene, v gene + aa
  ...
  # length(x_highCounts$data)
  # x_highCounts$data = x_highCounts$data[which(sapply(x_highCounts$data, nrow)!=0)] #drop any repertoires that are empty
  # length(x_highCounts$data)

  imm_ov3 <- repOverlap(x_highCounts$data, .method="morisita", .col="aa") 
  
 plot5<-  vis(imm_ov3, .text.size = 2, .signif.digits = 1)+
    ggtitle("Clones > 1")+
    my_xaxis+
   sc+
    scale_y_discrete(labels= function(y) x$meta$patient_tls_pub_id[match(y, x$meta$Sample)], 
  ...
  # x_lowCounts$data = x_lowCounts$data[which(sapply(x_lowCounts$data, nrow)!=0)] #drop any repertoires that are empty
  
  imm_ov4 <- repOverlap(x_lowCounts$data, .method="morisita", .col="aa")
  
  plot6<- vis(imm_ov4, .text.size = 2, .signif.digits = 1)+
    ggtitle("Clones = 1")+
    my_xaxis+
    sc+
    scale_y_discrete(labels= function(y) x$meta$patient_tls_pub_id[match(y, x$meta$Sample)], 
  ...
  rownames(imm_ov2) <- x$meta$new_name[match(rownames(imm_ov2), x$meta$Sample)]
  colnames(imm_ov2) <- x$meta$new_name[match(colnames(imm_ov2), x$meta$Sample)]
...
### Gene usage
gene.usage <- function(x) {
  plot1<- geneUsage(x$data) %>% vis()
  plot2<- lapply(1:length(meta.subset), function (i) 
    geneUsage(x$data) %>% 
...
diversity <- function(x){
  ...
  plot2<-lapply(1:length(meta.subset), function (i) vis(div_chao, .by = meta.subset[i], .meta = x$meta)+theme_prism())
  ...
  plot6<-lapply(1:length(meta.subset), function (i) vis(div_d50, .by = meta.subset[i], .meta = x$meta)+theme_prism())
  
  div_div <- repDiversity(x$data, "div")
  plot7<-vis(div_div)+my_xaxis+theme(legend.position="NULL")
  plot8<-lapply(1:length(meta.subset), function (i) vis(div_div, .by = meta.subset[i], .meta = x$meta)+theme_prism())
  ...
  plot10<-lapply(1:length(meta.subset), function (i) vis(div_div, .by = meta.subset[i], .meta = x$meta)+theme_prism()+ggtitle("div_div where q=1"))
